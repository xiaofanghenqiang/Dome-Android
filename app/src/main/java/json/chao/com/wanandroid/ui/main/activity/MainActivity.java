...
public class MainActivity extends BaseActivity<MainPresenter> implements MainContract.View {
    ...
    public void showLoginView() {
        ...
        mNavigationView.getMenu().findItem(R.id.nav_item_logout).setVisible(true);//让退出登录的itme显示
    }
    ...
    public void showLogoutView() {
        ...
        mNavigationView.getMenu().findItem(R.id.nav_item_logout).setVisible(false);//让退出登录的itme隐藏
    }
    ...
    private void initFragments() {      //知识层级的Fragment
    ...
    private void switchFragment(int position) {
        //通过position判断导航栏和 返回开头的按钮隐藏 和显示
    ...
    private void initNavigationView() {
        mUsTv = mNavigationView.getHeaderView(0).findViewById(R.id.nav_header_login_tv);
        if (mPresenter.getLoginStatus()) {//判断是否登录了
            showLoginView();
        } else {
            showLogoutView();
        }
